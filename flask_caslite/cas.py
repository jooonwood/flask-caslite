...
class CasClient:
    ...
    def validate_cas_v1(cas_server, service_url, ticket):
        xmldump = urlopen(cas_server + '/validate?ticket='+ticket+'&service='+service_url).read().strip().decode('utf8', 'ignore')  # noqa: E501
        info = xmldump.split('\n')
        if info[0] == 'yes':
    ...
    def validate_cas_v2(cas_server, service_url, ticket):
        try:
            xmldump = urlopen(cas_server + '/serviceValidate?ticket=' + ticket + '&service=' + service_url).read().strip().decode('utf8', 'ignore')  # noqa: E501
        ...
        if isValid:
            ...
            if attributes and 'cas:memberOf' in attributes:
                ...
                for group_number in range(0, len(attributes['cas:memberOf'])):
                    attributes['cas:memberOf'][group_number] = attributes['cas:memberOf'][group_number].lstrip(' ').rstrip(' ')  # noqa: E501
